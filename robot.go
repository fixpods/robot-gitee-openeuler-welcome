...
func (bot *robot) getMaintainers(org, repo, sigName string, number int32, cfg *botConfig, log *logrus.Entry) ([]string, []string, error) {
	if cfg.WelcomeSimpler {
		err, membersToContact := bot.findSpecialContact(org, repo, number, cfg, log)
...
func (bot *robot) findSpecialContact(org, repo string, number int32, cfg *botConfig, log *logrus.Entry) (error, sets.String) {
	if number == 0 {
		return nil, nil
	}
